...
def main() -> None:
    ...
    if run_button:
        ...
        live_wip_placeholder = st.empty()  # Placeholder for live WIP plot
        ...
        with st.spinner("Running simulation..."):
            ...
            def update_progress(event_type, event_count, total_count, wip_df=None):
                ...
                # NEW: Update live WIP plot
                if wip_df is not None and len(wip_df) > 0:
                    live_fig = create_live_wip_plot(wip_df)
                    live_wip_placeholder.pyplot(live_fig, clear_figure=True)
            ...
            event_details.empty()
            live_wip_placeholder.empty()  # NEW: Clear live plot
